...
class Lunar_agent:
    def __init__(self, state_size, action_size, seed, batch_size=64, gamma=0.99, learning_rate=1e-4,
        ...
        self.memory = N_Step_Replay_Buffer(action_size=self.action_size, capacity = self.capacity, batch_size = self.batch_size, seed = self.seed, n_step= self.n_step,
                                          gamma = self.gamma)
...
def train(n_episodes=2000, max_t=1000, eps_start=1.0, eps_end=0.01, eps_decay=0.9995):
    ...
    torch.save(lunar_agent.policy_net.state_dict(), 'checkpoint_lunar_agent_nstep11_V1.pth')
...
print('\n', df.head())
df.to_csv("scores_N_step_11V1.csv")
