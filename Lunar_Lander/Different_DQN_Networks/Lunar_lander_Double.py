# https://github.com/Shivanshu-Gupta/Pytorch-Double-DQN/blob/1cff44d95d7881c6afc029b734508b1a705dfe14/agent.py#L92
...
save_name = 'checkpoints/' + modelName
...
class Lunar_agent:
    ...
    def step(self, state, action, reward, next_state, done):
        ...
        if self.time_step == 0:
            ...
            if len(self.memory) > self.batch_size:
                ...
                self.train_Double_DQN(experiences, self.gamma)
    ...
    def train_Double_DQN(self, experiences, gamma):
        states, actions, rewards, next_states, dones = experiences
        # we need the max predicted Q values for next states from our target model

        # Instead of Q values from the target network, the selection of actions uses online weights,
        # contingent upon S_t+1 ; rather than S_t
        # in original DQN ; we only want actions from this


        _, dd_action_values = self.policy_net(next_states).detach().max(1)

        # Q_targets_net should utilize weights from target network, but action selection from policy network

        Q_targets_net = self.target_net(next_states).detach().gather(1, dd_action_values.unsqueeze(1))
...
print('\n', df.head())
df.to_csv("scores_DQN_Double_old_bad.csv")
