...
print('\n', df.head())
df.to_csv("scores_DQN_Dueling.csv")
print("saved the scores")
print(30 * '#' + ' Training the agent Finished ' + 30 * '#')
# let's plot the scores of the training procedure
fig = plt.figure()
...
lunar_agent_smart.policy_net.load_state_dict(torch.load('checkpoint_lunar_agent.pth', map_location = 'cpu'))

for i in range(3):
    state = env.reset()
    img = plt.imshow(env.render(mode = 'rgb_array'))
    for j in range(200):
        action = lunar_agent_smart.get_action(state, epsilon=0) # we take greedy for evaluation
