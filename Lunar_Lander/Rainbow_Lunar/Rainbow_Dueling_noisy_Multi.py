...
print("\n This is Rainbow dueling noisy with multisteps ! \n")
...
class Noisy_Dueling_DQN(nn.Module):
    ...
    def __init__(self, state_size, action_size,seed, fc1_size=128, fc2_size=64):
        super(Noisy_Dueling_DQN, self).__init__()
        ...
        # Value Noisy
        self.value_N1 = NoisyLinear(fc2_size, self.fc3_size)
        self.value_N2 = NoisyLinear(self.fc3_size, action_size)

        # Advantage Noisy
        self.advan_N1 = NoisyLinear(fc2_size, self.fc3_size)
        self.advan_N2 = NoisyLinear(self.fc3_size, action_size)

    def forward(self, state):
        x = F.relu(self.fc1(state))
        x = F.relu(self.fc2(x))
        # Value
        val = F.relu(self.value_N1(x))
        val = self.value_N2(val)
        # Advantage
        adv = F.relu(self.advan_N1(x))
        adv = self.advan_N2(adv)
...
class Lunar_agent:
    def __init__(self, state_size, action_size, seed, batch_size=64, gamma=0.99, learning_rate=1e-4,
        ...
        # Q-Network
        self.policy_net = Noisy_Dueling_DQN(state_size=self.state_size, action_size=self.action_size,
                                            seed = seed).to(device)
        self.target_net = Noisy_Dueling_DQN(state_size=self.state_size, action_size=self.action_size,
                                            seed = seed).to(device)
        if self.pretrained:
        ...
        self.memory = N_Step_Replay_Buffer(action_size=self.action_size, capacity=self.capacity,
    ...
    def step(self, state, action, reward, next_state, done):
        ...
        if self.time_step == 0:
            ...
            if len(self.memory) > self.batch_size:
                ...
                self.train_DDQN(experiences, self.gamma)
    ...
    def train_DDQN(self, experiences, gamma):
        ...
        _, a_prime = Q_argmx.max(1)
        # get max predicted Q Values from target model
        Q_targets_nxt_states = self.target_net(next_states).detach().gather(1, a_prime.unsqueeze(1))
        # calculate the Q targets from current states
        Q_targets = rewards + (gamma * Q_targets_nxt_states * (1 - dones))
        # Q from policy
...
print('\n', df.head())
df.to_csv("scores_Dueling_Noisy_multi.csv")
